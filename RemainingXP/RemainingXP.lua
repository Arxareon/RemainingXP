...
local presets = {
	...
	[6] = {
		name = ns.strings.options.display.quick.presets.list[5], --Objective Tracker Bar
		data = {
			position = {
				anchor = "TOPRIGHT",
				offset = { x = -92, y = -286, },
	...
	[7] = {
		name = ns.strings.options.display.quick.presets.list[6], --Menu & Bags Small Bar
		data = {
			position = {
				anchor = "BOTTOMRIGHT",
				offset = { x = -78, y = 32, },
...
local function GetBankedText()
	return (GameLimitedMode_IsActive() and csc.xp.banked > 0) and " + " .. ns.strings.xpBar.banked:gsub(
	...
	if GameLimitedMode_IsActive() and csc.xp.banked > 0 then
		textLines[#textLines + 1] = {
			text = "\n" .. ns.strings.xpTooltip.banked:gsub(
				"#DATA", wt.Color(ns.strings.xpTooltip.valueBanked:gsub(
					"#VALUE", wt.Color(wt.FormatThousands(csc.xp.banked), ns.colors.grey[1])
				):gsub(
					"#LEVELS", wt.Color(csc.xp.bankedLevels, ns.colors.grey[1])
				), ns.colors.grey[2])
			),
...
local function CreateOptionsShortcuts(parentFrame)
	...
	local display = wt.CreateButton({
		...
		tooltip = { lines = {
			[0] = { text = ns.strings.options.display.description:gsub("#ADDON", addonTitle), },
	...
	local integration = wt.CreateButton({
		...
		tooltip = { lines = {
			[0] = { text = ns.strings.options.integration.description:gsub("#ADDON", addonTitle), },
			[1] = { text = (wt.GetStrings("dfOpenSettings") or ""):gsub("#ADDON", addonTitle), color = { r = 1, g = 0.24, b = 0.13 }, },
		} },
		position = {
			relativeTo = display,
	...
	wt.CreateButton({
		...
		tooltip = { lines = {
			[0] = { text = ns.strings.options.events.description:gsub("#ADDON", addonTitle), },
			[1] = { text = (wt.GetStrings("dfOpenSettings") or ""):gsub("#ADDON", addonTitle), color = { r = 1, g = 0.24, b = 0.13 }, },
		} },
		position = {
...
local function CreateRemovalsOptions(parentFrame)
	...
	options.removals.statusBars = wt.CreateCheckbox({
		...
		tooltip = { lines = {
			[0] = { text = ns.strings.options.integration.removals.statusBars.tooltip[0]:gsub("#ADDON", addonTitle), },
			[1] = { text = ns.strings.options.integration.removals.statusBars.tooltip[1], color = { r = 1, g = 0.24, b = 0.13 }, },
		} },
		position = { offset = { x = 8, y = -30 } },
		autoOffset = true,
		events = { OnClick = function(_, state) wt.SetVisibility(StatusTrackingBarManager, not state) end, },
		optionsData = {
			optionsKey = addonNameSpace,
	...
	local importPopup = wt.CreatePopup({
		...
		onAccept = function()
			...
			if success and type(t) == "table" then
				...
				--Removals
				wt.SetVisibility(StatusTrackingBarManager, t.account.removals.statusBars)
...
local function CancelChanges()
	...
	--Removals
	wt.SetVisibility(StatusTrackingBarManager, db.removals.statusBars)
end
...
--Set up the main display context menu
local function CreateContextMenu(parent)
	...
	wt.AddContextButton(presetsMenu, contextMenu, {
		...
		events = { OnClick = function() PresetCommand(2) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[3].name,
		events = { OnClick = function() PresetCommand(3) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[4].name,
		events = { OnClick = function() PresetCommand(4) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[5].name,
		events = { OnClick = function() PresetCommand(5) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[6].name,
		events = { OnClick = function() PresetCommand(6) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[7].name,
		events = { OnClick = function() PresetCommand(7) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[8].name,
		events = { OnClick = function() PresetCommand(8) end, },
	})
	wt.AddContextButton(presetsMenu, contextMenu, {
		title = presets[9].name,
		events = { OnClick = function() PresetCommand(9) end, },
	})
end
...
local function SetUpIntegratedFrame()
	...
	--Frame & Text
	integratedDisplay:SetPoint("BOTTOM", StatusTrackingBarManager, "BOTTOM")
	integratedDisplay:SetFrameStrata("HIGH")
	integratedDisplay:SetToplevel(true)
	integratedDisplayText:SetPoint("CENTER", 0, 3)
	integratedDisplay:SetSize(StatusTrackingBarManager:GetWidth(), 17)
	SetIntegrationVisibility(db.enhancement.enabled, db.enhancement.keep, db.enhancement.remaining, true, false)
	--Context menu
	CreateContextMenu(integratedDisplay)
end
	...
	if GameLimitedMode_IsActive() then
		...
		if UnitLevel("player") >= rLevel then
			if csc.xp.banked > 0 then
				GameTooltip:SetOwner(StatusTrackingBarManager, "ANCHOR_RIGHT", 0, -14)
